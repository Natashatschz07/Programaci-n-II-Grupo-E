...
def enmascarar_fecha(texto):    
    ...
    formato_final=""                             #para guadar la fecha en un formato valido en tkinter
    
    if len(limpio)>8:
        limpio=limpio[:8]
    if len(limpio)>4:
        formato_final=f"{limpio[:2]}-{limpio[2:4]}-{limpio[4:]}" #ordena la fecha en 2 en 2
    elif len (limpio)>2:
...
#Lista de pacentes(inicialmente vacia)
paciente_data=[]
#Funcion para registara paciente
...
#Boton Registrar
btn_registrar=tk.Button(btn_frame,text="Registrar", command=registrarPaciente,bg="LimeGreen")
...
labelNombr.grid(row=0,column=0,padx=5,pady=5,sticky="w")
nombrP=tk.Entry(frame_doctores)
nombrP.grid(row=0,column=1,padx=5,pady=5,sticky="w")
...
telefP=tk.Entry(frame_doctores)
telefP.grid(row=4,column=1,padx=5,pady=5,sticky="w")
...
treeview.column("Edad", width=100, anchor="center")
treeview.column("Telefono", width=100, anchor="center")
